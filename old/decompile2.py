...
def get_separate_palettes(primary_path, secondary_path):
    """Loads palettes from both paths separately to allow prioritization."""
    p_pals = {}
    s_pals = {}
    
    for pf in glob.glob(os.path.join(primary_path, "palettes", "*.pal")):
        try:
            pal_id = int(os.path.basename(pf).split('.')[0])
            p_pals[pal_id] = load_jasc_pal_as_list(pf)
        except ValueError: continue
            
    for pf in glob.glob(os.path.join(secondary_path, "palettes", "*.pal")):
        try:
            pal_id = int(os.path.basename(pf).split('.')[0])
            s_pals[pal_id] = load_jasc_pal_as_list(pf)
        except ValueError: continue
            
    return p_pals, s_pals

def create_tileset_library(tiles_png_path, p_pals, s_pals, is_secondary_lib=False):
    """
    Creates a dict of {pal_id: RGBA_image}.
    is_secondary_lib: If True, allows secondary palettes to exist alongside primary.
    """
    if not os.path.exists(tiles_png_path): return {}
    base_img = Image.open(tiles_png_path).convert("P")
    library = {}
    
    # Create the palette priority map
    # We start with Primary palettes as the base
    merged_pals = p_pals.copy()
    
    if is_secondary_lib:
        # For the secondary tileset, add secondary palettes.
        # If IDs 6-12 exist in secondary folder, they take priority.
        # If IDs 0-5 exist in secondary folder, we IGNORE them to keep the water fix.
        for pal_id, pal_data in s_pals.items():
            if pal_id >= 6 or pal_id not in merged_pals:
                merged_pals[pal_id] = pal_data
    
    for pal_id, pal_data in merged_pals.items():
        version = base_img.copy()
        version.putpalette(pal_data)
        rgba_version = version.convert("RGBA")
        
        # Proper Index-0 Transparency
        indices = list(base_img.getdata())
        pixels = list(rgba_version.getdata())
        new_pixels = []
        for i in range(len(pixels)):
            if indices[i] == 0:
                new_pixels.append((pixels[i][0], pixels[i][1], pixels[i][2], 0))
            else:
...
def decompile_secondary(primary_path, secondary_path, out_dir):
    ...
    # 1. Load Palettes
    print("Loading palette sets...")
    p_pals, s_pals = get_separate_palettes(primary_path, secondary_path)

    # 2. Generate Libraries
    print("Generating Tileset Libraries...")
    # Primary tiles ONLY use Primary palettes
    primary_lib = create_tileset_library(os.path.join(primary_path, "tiles.png"), p_pals, {}, False)
    # Secondary tiles use Primary for 0-5 and Secondary for 6+
    secondary_lib = create_tileset_library(os.path.join(secondary_path, "tiles.png"), p_pals, s_pals, True)
    ...
    num_metatiles = len(attr_data) // 2
    img_w = 128
    mt_per_row = img_w // METATILE_SIZE
    ...
    for i in range(num_metatiles):
        ...
        mx, my = (i % mt_per_row) * METATILE_SIZE, (i // mt_per_row) * METATILE_SIZE
        ...
        for layer_idx, layer_name in enumerate(target_layers):
            ...
            for t in range(4):
                ...
                # Use Primary library for low indices, Secondary library for high indices
    ...
    print(f"Decompiled secondary tileset to {out_dir}")
