...
- **Compile Primary Tilesets**: Build primary tilesets from image layers (`bottom.png`, `middle.png` and `top.png`) and a CSV with metatile attributes.
...
- **Finding an optimal solution**: Option to find the solution that minimizes the number of palettes used. For example, with this option set to True, the primary tileset of Pokémon Emerald can be compiled with only 5 palettes, and a couple of unused slots in the remaining palettes.
